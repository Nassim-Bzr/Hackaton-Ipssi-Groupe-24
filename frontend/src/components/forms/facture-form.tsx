...
type FactureFormProps = {
  ...
  onChangeNomFournisseur: (value: string) => void
  onChangeIban: (value: string) => void
  onUpload: () => void
}
  ...
  return (
    <article className="space-y-2 border bg-background p-3 text-xs">
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
        ...
        <div className="flex flex-col gap-1">
          <label className="text-xs">
            ...
            <Input
              ...
              onChange={(event: React.ChangeEvent<HTMLInputElement>) => onChangeNomFournisseur(event.target.value)}
              className="mt-1 h-8 text-xs"
            />
          </label>
        </div>
        ...
        <div className="flex flex-col gap-1">
          <label className="text-xs">
            ...
            <Input
              value={iban ?? ""}
              onChange={(event: React.ChangeEvent<HTMLInputElement>) => onChangeIban(event.target.value)}
              className="mt-1 h-8 text-xs"
            />
          </label>
        </div>
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        >
          {uploadState === "uploading" ? "Envoi en cours..." : "Uploader cette facture"}
        </Button>
